...
def export_csv():
    ...
    try:
        ...
        outcsv.writerow(['id', 'eid', 'when_entered',
                         'when_left', 'was_helped', 'instructor_id'])
        visits = Visit.query.all()
        for visit in visits:
            outcsv.writerow([visit.id, visit.eid, visit.time_entered,
                             visit.time_left, visit.was_helped, visit.instructor_id])
        output_file.close()
        return True
...
def generate_graphs(range):
    if not export_csv():
        ...
        return render_template('reset_message', title="Error Occurred",
                               body='An error occurred when trying to collect statistics for this class.')
    ...
    filtered.loc[:, 'when_entered'] = filtered['when_entered'].apply(
        lambda x: x + tzoffset)
    filtered.loc[:, 'when_left'] = filtered['when_left'].apply(
        lambda x: x + tzoffset)
    ...
    min_in_line = filtered['time_in_line'].apply(
    ...
    helped.loc[:, 'instructor_id'] = helped['instructor_id'].map(
        renameInstructor)
    ...
    if range == 'all':
        ...
        plot_waiting_per_day(
            graphs, filtered, "Number of students per weekday (all-time)")
        plot_helped_per_TA(
            graphs, helped, "Number of students helped by TA (all-time)")
        num_helped = len(helped)
        num_removed = len(removed)
        avg_wait = helped['min_in_line'].mean()

    elif range == 'month':
        plot_time_waiting(graphs, filtered_last_month,
                          "Time waiting (last month)", 10)
        plot_waiting_per_day(graphs, filtered_last_month,
                             "Number of students per weekday (last month)")
        plot_helped_per_TA(graphs, helped_last_month,
                           "Number of students helped by TA (last month)")
        num_helped = len(helped_last_month)
        num_removed = len(removed_last_month)
        avg_wait = helped_last_month['min_in_line'].mean()

    elif range == 'week':
        plot_time_waiting(graphs, filtered_last_week,
                          "Time waiting (last week)", 10)
        plot_waiting_per_day(graphs, filtered_last_week,
                             "Number of students per weekday (last week)")
        plot_helped_per_TA(graphs, helped_last_week,
                           "Number of students helped by TA (last week)")
    ...
    global graph_dict
    graph_dict[range] = (now + tzoffset, graphs, avg_wait,
                         num_helped, num_removed)
...
def plot_helped_per_TA(graphs, data, title):
    ...
    if(len(counts) > 0):
        plt.bar(x=counts.index, height=counts.values,
                tick_label=counts.index, width=.5)
    graphs.append(mpld3.fig_to_html(fig))
    plt.close()
